...
class VoteHead(BaseModule):
    ...
    def __init__(self,
        ...
        if self.use_proposal:
            self.proposal_weight = nn.Sequential(nn.Conv1d(128, 128, 1), nn.BatchNorm1d(128), nn.ReLU(inplace=True), nn.Conv1d(128, 2, 1))
            self.proposal_message = EncoderLayer(128, 4)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        if self.use_proposal:
            weight = self.proposal_weight(features)
            results['weight_score'] = weight
            weight = torch.nn.functional.softmax(weight, dim=1)
            features = self.proposal_message(features.transpose(1,2).contiguous(), weight[:,[1],:]).transpose(1,2).contiguous() + features
    ...
    def loss(self,
        ...
        # calculate vote loss
        if self.vote_module.non_vote_loss:
            vote_loss, non_vote_loss = self.vote_module.get_loss(bbox_preds['seed_points'],
        ...
        if 'weight_score' in bbox_preds.keys():
            score_loss = self.objectness_loss(
                bbox_preds['weight_score'],
                objectness_targets,
                weight=objectness_weights)
