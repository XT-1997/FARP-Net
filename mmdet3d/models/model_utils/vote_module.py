...
class VoteModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.non_vote_loss = non_vote_loss
    ...
    def get_loss(self, seed_points, vote_points, seed_indices,
        ...
        if self.non_vote_loss:
            # non-voting loss
            background_gt_votes_mask = 1 - seed_gt_votes_mask
            background_weight = background_gt_votes_mask / (torch.sum(background_gt_votes_mask) + 1e-6)
            non_vote_loss = torch.sum(torch.norm(vote_points.view(batch_size * num_seed, 3), dim=1) * background_weight.view(batch_size * num_seed))
            return vote_loss, non_vote_loss
        else:
            return vote_loss
