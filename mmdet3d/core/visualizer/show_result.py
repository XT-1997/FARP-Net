...
def show_result(points,
    ...
    if gt_bboxes is not None:  # gt_bboxes (N, 7)
        ...
        if gt_labels is None:
            ...
            for j in range(len(gt_labels)):
                i = int(gt_labels[j])
                if gt_labelDict.get(i) is None:
                    gt_labelDict[i] = []
                gt_labelDict[i].append(gt_bboxes[j])
            for i in gt_labelDict:
                bbox3d = np.array(gt_labelDict[i])
                _write_oriented_bbox(bbox3d,
                                     osp.join(result_path, f'{i}_gt.obj'))
    ...
    if pred_bboxes is not None:
        ...
        if pred_labels is None:
            ...
            for i in labelDict:
                bbox3d = np.array(labelDict[i])
                _write_oriented_bbox(bbox3d,
                                     osp.join(result_path, f'{i}_pred.obj'))
